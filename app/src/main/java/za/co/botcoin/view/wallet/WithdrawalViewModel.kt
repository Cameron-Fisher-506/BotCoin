...
import za.co.botcoin.model.repository.SendRepository
...
class WithdrawalViewModel(application: Application) : AndroidViewModel(application) {
    ...
    private val sendRepository: SendRepository = SendRepository(application)
    ...
    fun send(mustSend: Boolean, amount: String, currency: String, address: String, destinationTag: String = "") {
        sendLiveData = sendRepository.send(mustSend, amount, currency, address, destinationTag)
    }
}